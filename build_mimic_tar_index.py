...
def modify_image_paths(input_csv, tar_index, output_csv):
    """
    ...
    df.loc[:, "offset"] = df["tar_path"].apply(get_offset)
    df.loc[:, "size"] = df["tar_path"].apply(get_size)
...
def main():
    ...
    input_csv = root_dir / "geraugi/plural/pre_processed_data/xplainer_dataset.csv"  # This CSV must include a column 'tar_path'
    ...
    if not index_file.exists():
        ...
        print("Loading existing tar index...")
        tar_index = load_index(index_file)
    
    modify_image_paths(input_csv, tar_index, output_csv)
