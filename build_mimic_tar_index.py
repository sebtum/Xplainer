...
def main():
    ...
    if single_process:
        if post_process:
            tar_file_path = root_dir / f"geraugi/plural/dataset_files/resized_ref_images-2.0.0.tar"
            input_csv = root_dir / "geraugi/plural/dataset_files/500_ref_xplainer_mimic_dataset.csv"
            output_csv = root_dir / "geraugi/plural/dataset_files/correct_500_ref_xplainer_mimic_dataset.csv"
            index_file = root_dir / f"geraugi/plural/dataset_files/resized_ref_mimic_tar_indexes.json"
            path_header = "member_path"
        else:
            if ref_flag:
                ref = "ref_"
            else:
            ...
            input_csv = root_dir / f"geraugi/plural/pre_processed_data/{ref}xplainer_dataset.csv"
            output_csv = root_dir / f"geraugi/plural/pre_processed_data/{ref}xplainer_mimic_dataset.csv"
            index_file = root_dir / f"geraugi/plural/dataset_files/mimic_tar_indexes.json"
            path_header = "tar_path"

        # If the index file does not exist, build it and save it.
        if not index_file.exists():
            print(f"Building tar index {index_file} from {tar_file_path}...")
            tar_index = build_tar_index(tar_file_path)
            save_index(tar_index, index_file)
        else:
            print(f"Loading existing tar index {index_file}...")
            tar_index = load_index(index_file)
        
        modify_image_paths(input_csv, tar_index, output_csv, path_header)
    else:    
        for i in range(8):
            part = f"part{i}"
